@php
    ...
    $method = Illuminate\Support\Str::camel(sprintf('%s_relationship', $row->field)); // for example:  subcategoriaIdRelationship
    $primaryKey = app($model_name)
        ->make()
        ->getKeyName();
...
@if (method_exists($model, $method))
    ...
    @php
        $model = $model::{$method}($dataTypeContent->$primaryKey); // for example:  App\Producto::subcategoriaIdRelationship($productId)
    ...
    @if (!empty($data))
        ...
        <div class="{{ $css }}" style="padding: 10px;">
            ...
            <select id="{{ $row->details->name }}" name="{{ $row->details->name }}"
                class="form-control select2 dependent-dropdown" data-route="{{ route($row->details->route) }}"
                ...
                @foreach ($query as $key => $option)
                    <option value="{{ $key }}"
                        @if ($key == $data[$row->details->name]) {{ 'selected="selected"' }} @endif>{{ $option }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        @foreach ($selects as $key => $item)
            @php
                $query = app($item->model)
                    ::where($item->where, '=', $data[$item->where])
                    ->pluck($item->label, $item->key);
                $next = ++$key;
            @endphp
            <div class="{{ $css }}" style="padding: 10px;">
                @if (isset($selects[$next]))
                    ...
                    <select id="{{ $item->name }}" name="{{ $item->name }}"
                        class="form-group select2 dependent-dropdown" data-route="{{ route($row->details->route) }}"
                        ...
                        @foreach ($query as $k => $option)
                            <option value="{{ $k }}"
                                @if ($k == $data[$item->name]) {{ 'selected="selected"' }} @endif>
                                {{ $option }}</option>
                        @endforeach
                    </select>
                @else
...
@else
    <p class="label label-warning"><i class="voyager-warning"></i>
        {{ __('voyager::form.field_select_dd_relationship', ['method' => 'public static function ' . Illuminate\Support\Str::camel($method) . '($id){}', 'class' => $dataType->model_name]) }}
    </p>
@endif
