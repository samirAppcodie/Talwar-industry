@php
    ...
    // dd($row->details);
    $reference_field = $row->details->reference_field ?? null;
    $errors = [];
    if (!class_exists($row->details->model)) {
        $errors[] = $row->details->model;
    } // check error

    $selects = $row->details->dependent_dropdown;
    foreach ($selects as $class) {
    ...
    $selectName = $reference_field ? "{$reference_field}[{$row->details->name}]" : $row->details->name;
@endphp
...
@if (!count($errors))
    ...
    <div class="{{ $css }}" style="padding: 10px;">
        ...
        <select id="{{ $row->details->name }}" name="{{ $selectName }}" class="form-group select2 dependent-dropdown"
    ...
    @foreach ($selects as $key => $item)
        @php
            $next = ++$key;
            $selectName = $reference_field ? "{$reference_field}[{$item->name}]" : $item->name;

        @endphp
        <div class="{{ $css }}" style="padding: 10px;">
            @if (isset($selects[$next]))
                <label for="{{ $item->name }}">{{ $item->display }}</label>
                <select id="{{ $item->name }}" name="{{ $selectName }}"
            ...
            @else
                <label for="{{ $item->name }}">{{ $item->display }}</label>
                <select id="{{ $item->name }}" name="{{ $selectName }}" class="form-group select2"
                    placeholder="{{ $item->placeholder }}">
                    @if (isset($item->placeholder))
                        <option value="0" selected="selected">{{ $item->placeholder }}</option>
                    @endif
