...
class customer_priceController extends VoyagerBaseController
{
    ...
    public function dependent_prices_relationship($id)
    {
        $setting = CustomerPriceSetting::find($id);
        if (!$setting) {
            return null;
        }

        $wheatPrice = WheatPrice::find($setting->wheat_price_per_kg);
        $grindingCharge = GrindingCharge::find($setting->grinding_charge_per_kg);

        return [
            'wheat_price_per_kg' => $wheatPrice ? $wheatPrice->id : null,
            'grinding_charge_per_kg' => $grindingCharge ? $grindingCharge->id : null,
            'customer_type' => $setting->customer_type,
        ];
    }
}
